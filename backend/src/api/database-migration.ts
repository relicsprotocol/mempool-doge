...
class DatabaseMigration {
  ...
  private async $createMissingTablesAndIndexes(databaseSchemaVersion: number) {
    ...
    const isBitcoin = ['mainnet', 'testnet', 'signet', 'doge'].includes(config.MEMPOOL.NETWORK);
  ...
  private getAdditionalBlocksDataQuery(): string {
    return `ALTER TABLE blocks
      ...
      ADD segwit_total_txs int unsigned NULL,
      ADD segwit_total_size int unsigned NULL,
      ADD segwit_total_weight int unsigned NULL,
      ADD header varchar(4096) NOT NULL,
