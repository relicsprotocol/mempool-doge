...
class BitcoinRoutes {
  ...
  private async getBlocks(req: Request, res: Response) {
    try {
      if (['mainnet', 'testnet', 'signet', 'doge'].includes(config.MEMPOOL.NETWORK)) { // Bitcoin
  ...
  private async getBlocksByBulk(req: Request, res: Response) {
    try {
      if (['mainnet', 'testnet', 'signet', 'doge'].includes(config.MEMPOOL.NETWORK) === false) { // Liquid - Not implemented
