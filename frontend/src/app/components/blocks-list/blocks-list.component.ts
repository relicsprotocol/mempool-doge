...
export class BlocksList implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.widget) {
      ...
      this.seoService.setDescription($localize`:@@meta.description.dogecoin.blocks:See the most recent Dogecoin${seoDescriptionNetwork(this.stateService.network)} blocks along with basic stats such as block height, block reward, block size, and more.`);
