...
export class BlockFeesSubsidyGraphComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.seoService.setDescription($localize`:@@meta.description.dogecoin.graphs.block-fees-subsidy:See the mining fees earned per Dogecoin block compared to the Dogecoin block subsidy, visualized in DOGE and USD over time.`);
      ...
      tooltip: {
        ...
        formatter: function (data) {
          ...
          for (let i = data.length - 1; i >= 0; i--) {
            ...
            if (this.displayMode === 'normal') tooltip += `${formatNumber(tick.data, this.locale, '1.0-3')} DOGE<br>`;
          ...
          if (this.displayMode === 'normal') tooltip += `<div style="margin-left: 2px">${formatNumber(data.reduce((acc, val) => acc + val.data, 0), this.locale, '1.0-3')} DOGE</div>`;
      ...
      yAxis: this.data.blockFees.length === 0 ? undefined : [
        {
          ...
          axisLabel: {
            color: 'var(--grey)',
            formatter: (val) => {
              return `${val}${this.displayMode === 'percentage' ? '%' : ' DOGE'}`;
            }
          },
          min: 0,
          max: (value) => {
